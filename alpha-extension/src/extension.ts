...
interface RC   { assignmentId: string; token: string; username: string; }
interface Auth { username: string; userId: string; }

function getRC(): RC | null {
...
function detectProjectType(wsPath: string): 'html' | 'vite' | 'react' | 'node' | 'unknown' {
  const has = (f: string) => fs.existsSync(path.join(wsPath, f));
  if (has('vite.config.js') || has('vite.config.ts')) return 'vite';
  if (has('src/App.jsx') || has('src/App.tsx'))         return 'react';
  if (has('index.html') && !has('package.json'))        return 'html';
  if (has('index.html') && has('package.json'))         return 'vite';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (!auth) authBtn.backgroundColor = new vscode.ThemeColor('statusBarItem.errorBackground');
  authBtn.show();
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('alphalearn.panel', provider)
  );

  // ── EVALUATE ────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('alpha.evaluate', () => {
      const rc = getRC();
      if (!rc) {
  ...
  // ── SUBMIT ──────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('alpha.submit', async () => {
      const rc = getRC();
      if (!rc) {
      ...
      const ans = await vscode.window.showWarningMessage(
        `Submit Assignment: ${rc.assignmentId}?`,
        { modal: true },
        'Yes, Submit', 'Cancel'
      );
      if (ans !== 'Yes, Submit') return;
      const t = getTerminal('AlphaLearn');
      const ws = vscode.workspace.workspaceFolders![0].uri.fsPath;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('alpha.preview', async () => {
      const folders = vscode.workspace.workspaceFolders;
      if (!folders) {
...
class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView) {
    ...
    view.webview.onDidReceiveMessage(m =>
      vscode.commands.executeCommand(`alpha.${m.cmd}`)
    );
  }
  ...
  getHtml(webview: vscode.Webview): string {
    ...
    const logoUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.extUri, 'resources', 'logo.png')
    );
